...
import { ISmscSession } from '../../smsc-session/smsc-session.model';
...
export class SmscInstanceDetailComponent implements OnInit {
  ...
  loadAll(instanceID: string): void {
    this.httpClient
      .get<ISmscSession[]>(`api/smsc-instance/${instanceID}/sessions`, { observe: 'response' })
      .subscribe((res: HttpResponse<ISmscSession[]>) => (this.smscSessions = res.body ?? []));
  }
  ...
  startSmscInstance(instanceID: string): void {
    this.httpClient.get(`api/smsc-instances/${instanceID}/start`).subscribe();
  }

  stopSmscInstance(instanceID: string): void {
    this.httpClient.delete(`api/smsc-instances/${instanceID}/stop`).subscribe();
  }
  ...
  sendSms(accountID: string): void {
    this.performConvertText().subscribe(() => {
      const baseSm = this.createFromForm();
      this.httpClient.post(`api/smsc-accounts/${accountID}/send-mt`, baseSm).subscribe();
    });
  }
  ...
  sendSmsOnSession(sessionID: string): void {
    this.performConvertText().subscribe(() => {
      const baseSm = this.createFromForm();
      this.httpClient.post(`api/smsc-sessions/${sessionID}/send-mt`, baseSm).subscribe();
    });
  }

  deleteSmscSession(sessionID: string): void {
    this.httpClient.delete(`api/smsc-sessions/${sessionID}`).subscribe(() => this.loadAll(this.smscInstance!.id!));
  }
